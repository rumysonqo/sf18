...
<div id="sf18">
    <div class="container-fluid">
        ...
        <div class="form-group row">
            <label for="prg" class="col-lg-1 col-form-label">Programa</label>
            <div class="col-lg-4">
                <select name="prg" v-model="gen_sel" class="custom-select custom-select-md" v-on:change.prevent="getPresupuesto_fuente_generica(), getPpto_fuente_generica()">
                    <option disabled value="" selected>Selecciona un programa</option>
                    <option v-for="item in programa" v-bind:value="{id:item.id_generica, text:item.nombre_generica}">
                        @{{item.nombre_generica}}
                    </option>
                </select>
            </div>
